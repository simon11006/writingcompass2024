...
문단을 나누고 싶을 때는 Enter키를 한 번 눌러주세요." 
        ...
        <div id="result" className="container mx-auto px-4 hidden">
          {/* 분석 결과가 여기에 표시됩니다 */}
        </div>
        ...
        <div className="tip-modal hidden" id="tipModal">
          ...
          <button 
            ...
            onClick={() => document.getElementById('tipModal').classList.add('hidden')}
          >
            알겠어요!
          </button>
        </div>
        <div className="modal-overlay hidden" id="modalOverlay" onClick={() => {
...
async function getParagraphSuggestions() {
  ...
  try {
    ...
    suggestionButton.innerHTML = '<span>문단 제안하기</span><span className="button-icon">💡</span>';
  }
}
...
function toggleParagraphPreview() {
  ...
  if (previewContent.classList.contains('hidden')) {
    ...
    let previewHTML = '<div className="preview-paragraphs">';
    ...
    previewButton.innerHTML = '<span>미리보기 닫기</span><span className="button-icon">✕</span>';
  } else {
    previewContent.classList.add('hidden');
    previewButton.innerHTML = '<span>문단 미리보기</span><span className="button-icon">👁️</span>';
  }
}
...
function displayAnalysis(analysisContent, essayData) {
  ...
  result.innerHTML = '';  // 이전 분석 결과를 지웁니다
